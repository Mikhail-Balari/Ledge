...
def run_cli_ci_check(*paths):
    return subprocess.run(
        [sys.executable, "-m", "ledge_lang.cli", "ci-check", *map(str, paths)],
        cwd=ROOT,
        capture_output=True,
        text=True,
        timeout=30,
...
def test_installed_cli_ci_check_passes_required_paths():
    result = run_cli_ci_check(
        ROOT / "ledge_lang" / "demos",
        ROOT / "examples" / "python_integration",
    )
...
def test_installed_cli_ci_check_fails_invalid_ledge_file(tmp_path):
    bad = tmp_path / "unsafe.ledge"
    bad.write_text(
        "\n".join(
            [
