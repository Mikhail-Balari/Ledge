...
## Version 0.1 — Reference Document
...
This document is the authoritative specification of the Ledge programming language. It defines the syntax, semantics, type system, standard library interface, and execution model.

### 1.2 Design Philosophy

Ledge was designed from one observation: AI systems generate code all day, in millions of sessions, in languages designed for humans typing on keyboards in the 1980s and 1990s. Python carries the syntactic choices of 1991. JavaScript carries the memory of Netscape's ten-day sprint. These languages were designed for fingers, not for inference.

Ledge is the first language designed for the inverse order of priority:

1. **Generated by AI** — every construct has exactly one canonical form
2. **Read by humans** — every statement reads as spoken English
3. **Safe by construction** — every fallible operation requires an explicit fallback
4. **AI-native** — calling an inference model is a language primitive, not a library
5. **Portable** — one runtime, every platform from Arduino to cloud

### 1.3 Scope

This specification covers Ledge version 0.1. It defines what conforming Ledge programs mean, not how they are implemented.
...
The complete formal grammar is in `docs/GRAMMAR.md`. This section provides a condensed reference.
...
Programs written for Ledge 0.x may require changes to run on Ledge 1.0 (the first stable release). After 1.0, backward compatibility is guaranteed within a major version.
